...
from ._validation import validate_positive_float


def _prepare_position(position) -> np.ndarray:
...
def _prepare_centers(centers, dimension: int) -> np.ndarray:
    ...
    if centers.size % dimension != 0:
        raise ValueError("centers cannot be reshaped to the position dimension.")
...
def _active_differences(position, centers, sigma, cutoff):
...
def gaussian_bias_value(
    ...
    position = _prepare_position(position)
    height = float(height)
    ...
    if len(centers) == 0 or height == 0.0:
        return 0.0
    differences = _active_differences(position, centers, sigma, cutoff)
    if len(differences) == 0:
        return 0.0
    squared_distances = np.einsum("ij,ij->i", differences, differences)
    return float(
        np.sum(height * np.exp(-0.5 * squared_distances / sigma**2))
    )
